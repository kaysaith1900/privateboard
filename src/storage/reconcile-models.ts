...
export function reconcileAgentModels(opts: ReconcileOptions = {}): ReconcileResult {
  ...
  for (const agent of listAllAgents()) {
    ...
    if (primary && carrier) {
      ...
      // pool so each seat shows a different brand badge — on a multi-
      // model carrier this means a visibly mixed cast of brands; on
      // a single-model carrier the pool naturally collapses to that
      // vendor's own fast set.
