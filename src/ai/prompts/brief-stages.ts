 ...
 *  dimension across labelled items. Rendered through the kami-chart
 *  inline-SVG pipeline (`renderKamiBar()` in public/report.html). Use
 *  when the room produced a comparable measure (cost, support strength,
 *  time-to-ship, market size) across discrete named items. */
export interface BarChartVisual {
  type: "bar-chart";
  title: string;
  /** Y-axis caption (the quantity being measured). ≤ 32 chars. */
  yLabel: string;
  /** Optional unit · appended to the data label above each bar by the
   *  renderer (e.g. "44w"). Empty string is fine. ≤ 4 chars. */
  unit: string;
  bars: {
    /** Item name on the X axis. ≤ 24 chars. The renderer truncates to 18. */
    label: string;
    /** Numeric reading · bare number. The renderer auto-scales the Y axis
     *  to the next nice round value. */
    value: number;
  }[];
}
 ...
 *  through the kami-chart inline-SVG pipeline (`renderKamiTimeline()`
 *  in public/report.html). Strong fit for retro / historical-analogue
 *  / first-round-essay registers; opportunistic everywhere else. */
...
/** Pie/donut chart · 2–6 slices showing a distribution. Rendered as
 *  a DONUT through the kami-chart inline-SVG pipeline
 *  (`renderKamiDonut()` in public/report.html). The renderer sorts
 *  slices largest-first and prints percentages in the legend, so raw
 *  counts and percentages both work. The room's typical hits:
...
const SCAFFOLD_SYSTEM = [
  ...
  "",
  "8. **Visuals** · 0–4 blocks. Content-driven. **Strongly prefer chart sub-types over text matrices** — charts render as a real diagram readers can scan in seconds; text matrices are dense tables that take paragraphs to absorb. Only fall back to text matrix when the data shape genuinely doesn't fit any chart form.",
  "",
  "   Chart sub-types (strongly preferred · all rendered as inline SVG through the `kami-chart` pipeline):",
  "   · `quadrant-chart`    — items plotted on two real axes. Effort/impact, support/strength, urgency/uncertainty. Rendered as inline SVG via the `kami-chart` pipeline.",
  "   · `bar-chart`         — 2–8 named items ranked by ONE quantitative dimension · cost / support / size / time. Rendered as inline SVG via the `kami-chart` pipeline (spine-tokenised, no JS runtime).",
  "   · `timeline`          — 3–8 dated points telling a narrative arc · retro / historical analogue / projected sequence. Rendered as inline SVG via the `kami-chart` pipeline.",
  "   · `pie-chart`         — 2–6 slices showing a distribution · scenario probabilities / lens shares / vote tallies / market mix. Rendered as a DONUT (hollow centre carries the largest slice's value) via the `kami-chart` pipeline. Numbers can be percentages OR raw counts — the renderer normalises.",
  "",
...
const WRITE_SYSTEM = [
  ...
  "    For `quadrant-chart`:",
  "      ### {title}",
  "      Render a fenced ```kami-chart block · strict JSON. Two-axis plot where the top-right (Q1) quadrant is the brand's \"preferred\" zone, tinted in the spine accent. Schema:",
  "      ```",
  "      {",
  "        \"type\": \"quadrant-chart\",",
  "        \"title\": \"{scaffold.title}\",",
  "        \"xLabel\": \"{scaffold.xLabel · low→high}\",",
  "        \"yLabel\": \"{scaffold.yLabel · low→high}\",",
  "        \"q1\": \"{top-right · preferred}\",",
  "        \"q2\": \"{top-left}\",",
  "        \"q3\": \"{bottom-left}\",",
  "        \"q4\": \"{bottom-right}\",",
  "        \"focal\": \"{label of the load-bearing item · optional}\",",
  "        \"items\": [",
  ...
  "        · `x` and `y` are decimals strictly inside `(0, 1)` — 2 decimals max. Never use 0 or 1 exactly (points clip on the frame).",
  "        · `q1`–`q4` are short quadrant labels (≤ 24 chars). `q1` is the preferred / focal quadrant by convention — pick names accordingly.",
  "        · `focal` is OPTIONAL. When set, that item renders solid accent + larger radius — concentrates attention on the load-bearing position. Only one focal per chart.",
  "        · `tier` (optional, 1–4) downranks an item's visual weight: 1 = standard, 2 = near-focal (accent outline), 3 = standard, 4 = faint (muted outline). Use sparingly to push uninteresting positions to the background.",
  "        · 2–16 items. Item labels ≤ 24 chars; avoid colons / brackets.",
  ...
  "    For `bar-chart`:",
  "      ### {title}",
  "      Render a fenced ```kami-chart block · strict JSON. The renderer turns it into an inline SVG bar chart styled by the active spine palette:",
  "      ```",
  "      {",
  "        \"type\": \"bar-chart\",",
  "        \"title\": \"{scaffold.title}\",",
  ...
  "        \"unit\": \"{scaffold.unit}\",",
  "        \"focal\": \"{label of the one bar that carries the argument · optional}\",",
  "        \"bars\": [",
  "          { \"label\": \"{bar.label}\", \"value\": {bar.value} },",
  "          { \"label\": \"{bar.label}\", \"value\": {bar.value} }",
  "        ],",
  "        \"caption\": \"{One-sentence editorial gloss · what the chart says, not what it plots. Use *italic markers* (e.g. *2× faster*) for the operative phrase.}\"",
  "      }",
  "      ```",
  "      Hard rules:",
  "        · Strict JSON (double-quoted keys + string values, no trailing commas, no comments). The renderer parses and falls back to an error placeholder if JSON is malformed.",
  "        · 2–8 bars. Below 2 isn't a comparison; above 8 stops being scannable.",
  "        · `value` is a bare number (no quotes, no unit suffix). Y-axis auto-scales to the next nice round number.",
  "        · `unit` (e.g. \"w\", \"%\", \"$M\") is appended to the data label above each bar — keep it short (≤ 4 chars).",
  "        · `focal` is optional. When set, that bar renders in the spine accent and the others in neutral ink-mid — concentrates attention on the bar that carries the argument. When omitted, every bar gets the accent.",
  "        · `caption` is rendered in serif italic below the chart. Wrap the operative phrase in `*…*` so the renderer can italicise it in the spine accent.",
  "        · Labels ≤ 24 chars; the renderer truncates to 18 at draw time. Avoid colons / brackets in labels (cosmetic, not strict).",
  "        · NO blank lines inside the fenced block.",
  "",
  "    For `timeline`:",
  "      ### {title}",
  "      Render a fenced ```kami-chart block · strict JSON. Horizontal time-axis chart where events alternate above / below the axis; the `focal` event renders in the spine accent. Schema:",
  "      ```",
  "      {",
  "        \"type\": \"timeline\",",
  "        \"title\": \"{scaffold.title}\",",
  "        \"focal\": \"{period string of the load-bearing milestone · optional}\",",
  "        \"points\": [",
  ...
  "        ],",
  "        \"caption\": \"{Editorial gloss · what the arc says, with *italic* on the turn.}\"",
  "      }",
  "      ```",
  "      Hard rules:",
  "        · Strict JSON. 3–8 points. Below 3 is a stub; above 8 overflows the strip.",
  "        · `period` is the column header (mono): \"2019\", \"Q3 2024\", \"Today\", \"+12 mo\". ≤ 16 chars.",
  "        · `label` is the headline event (sans): ≤ 60 chars. Concrete; avoid corporate verbs.",
  "        · `description` is optional (≤ 40 chars after rendering — gets truncated). Empty string is fine.",
  "        · `focal` matches the `period` string of the load-bearing event. The matched event renders in accent: filled dot, solid connector, accent-bordered card. Only one focal.",
  "        · NO blank lines inside the fenced block.",
  "",
  "    For `pie-chart`:",
  "      ### {title}",
  "      Render a fenced ```kami-chart block · strict JSON. Renders as a DONUT (hollow centre with the largest slice's value + label inside). The largest slice carries the spine accent; smaller slices cascade down the neutral ramp. Schema:",
  "      ```",
  "      {",
  "        \"type\": \"donut\",",
  "        \"title\": \"{scaffold.title}\",",
  "        \"centerLabel\": \"{optional · short text under the centre value · falls back to the largest slice's label}\",",
  "        \"slices\": [",
  ...
  "      ```",
  "      Hard rules:",
  "        · `type` MUST be `donut` (the old `pie-chart` is renamed at render time).",
  "        · Strict JSON. 2–6 slices. Above 6 the chart stops being readable.",
  "        · `value` is a bare number — percentages summing to ~100 OR raw counts. The renderer normalises and prints percentages in the legend.",
  "        · The renderer SORTS slices largest-first, so the visual order may differ from the JSON order. Don't try to control colours by ordering — the accent always lands on the largest slice.",
  "        · `centerLabel` is OPTIONAL — defaults to the largest slice's label (uppercased). Set it when the slice label is long or wants a different framing inside the donut.",
  "        · Slice labels ≤ 24 chars. NO blank lines inside the fenced block.",
  "        · NO blank lines inside the fenced block. Indent body lines 4 spaces.",
  "",
  "## Inline charts · additional chart types beyond the typed visuals",
  "",
  ...
  "",
  "All inline charts render through the `kami-chart` pipeline · strict JSON inside a ```kami-chart fence, spine-tokenised inline SVG, no JS runtime. The catalogue below covers every inline chart type — `flowchart` / `mindmap`→`tree` / `gantt` / `sequenceDiagram`→`swimlane` / `journey`→`swimlane` / `stateDiagram-v2`→`state-machine`.",
  "",
  "**Chart bias · prefer charts where they fit naturally — no fixed quota.** Two sources count:",
  "  · Typed visuals where the sub-type is a chart (`quadrant-chart` / `bar-chart` / `timeline` / `pie-chart`).",
  "  · Inline chart blocks from the catalogue below (`flowchart` / `mindmap` / `gantt` / `sequenceDiagram` / `journey` / `stateDiagram-v2`).",
  ...
  "  · `Critical Assumptions` → ◇ `flowchart` ONLY when assumptions form a multi-step dependency chain WITH branching (assumption A holds → either B or C → recommendation D), 5+ nodes. Linear 3-step dependency = prose.",
  "  · `Scenario Tree` → ✓ `flowchart` when there are ≥ 3 scenarios EACH with named effects/triggers as sub-nodes (root + 3 scenarios + 3-6 effect children = 7-10 nodes). Below that, the typed table is enough.",
  "  · `Threats to Validity` → ◇ `flowchart` ONLY when threats compound (sample bias → selection bias → generalizability ceiling) with branching, 5+ nodes.",
  ...
  "  · `Risk Register` → ✓ `flowchart` when there are ≥ 5 risks AND multiple risks share a category cluster (root → category nodes → individual risks as leaves = ≥ 10 nodes). The typed risk table is enough on its own when the register is < 5 entries.",
  ...
  "  · A single section gets at MOST one inline chart (plus the typed visual if any). Never stack 2+ inline charts in one section.",
  ...
  "    Linear chain · 2–5 nodes top-to-bottom:",
  "    ```",
  "    {",
  "      \"type\": \"flowchart\",",
  "      \"layout\": \"linear-v\",",
  "      \"focal\": \"{optional · label of the load-bearing node}\",",
  "      \"nodes\": [",
  ...
  "    }",
  "    ```",
  "    Y-decision · root (decision) → 2 branches → optional join:",
  "    ```",
  "    {",
  "      \"type\": \"flowchart\",",
  "      \"layout\": \"y-decision\",",
  "      \"focal\": \"{optional · label of the load-bearing node — usually the decision or the preferred branch}\",",
  "      \"nodes\": [",
  ...
  "    ```",
  "  Hard rules:",
  "    · `layout` MUST be `linear-v` or `y-decision`. Pick `linear-v` for a sequence (\"A leads to B leads to C\"). Pick `y-decision` whenever a decision branches into 2 outcomes.",
  "    · `kind` values: `start` (pill, paper-soft) · `step` (square, ink border) · `decision` (diamond, accent border) · `outcome` (square, faint border — \"deferred branch\" reads as receded) · `end` (pill, paper-soft).",
  "    · `hint` is rendered as a short uppercase mono caption inside the node (for steps / outcomes) OR as the edge label (for y-decision branches — write `yes` / `no` / `<6 chars`).",
  "    · For `y-decision`: nodes in canonical order — [0] root, [1] left branch, [2] right branch, [3] optional join.",
  "    · For `linear-v`: max 5 nodes. For `y-decision`: 3–4 nodes.",
  "    · Labels ≤ 18 chars (renderer truncates). Avoid colons / brackets in labels.",
  "    · `focal` matches a node's `label` exactly; that node renders in the spine accent.",
  "    · **Complexity floor stays**: only emit a flowchart when prose can't carry the structure — a 2-node linear-v is exactly the trivial case banned above.",
  "",
  "  ### mindmap · hierarchical idea tree (renders as `tree`)",
  ...
  "      \"focal\": \"{optional · branch label that carries the argument}\",",
  "      \"branches\": [",
  ...
  "    ```",
  "  Hard rules:",
  "    · `type` MUST be `tree` (the old `mindmap` is renamed at render time).",
  "    · 2–6 branches. Each branch can have 0–4 leaves (no leaves is fine — the renderer just shows the branch tier).",
  "    · `root` ≤ 22 chars, branch labels ≤ 18 chars, leaf labels ≤ 16 chars. The renderer truncates.",
  "    · `focal` matches a branch label; that branch + its leaves render in the spine accent.",
  "    · **Complexity floor stays**: 2-branch tree with no leaves = naive. The renderer enforces a minimum of 2 branches but the floor in the trigger map applies: ≥ 4 top-level branches OR ≥ 3 branches × ≥ 2 children each.",
  "",
  ...
  "    {",
  "      \"type\": \"gantt\",",
  "      \"title\": \"Rollout phasing · 12 months\",",
  "      \"unit\": \"mo\",",
  "      \"focal\": \"{optional · phase label that carries the most cost / time}\",",
  "      \"phases\": [",
  ...
  "      ],",
  "      \"caption\": \"{Editorial gloss · what the schedule says, with *italic* on the operative phrase.}\"",
  "    }",
  "    ```",
  "  Hard rules:",
  "    · `start` and `end` are bare numbers in the same unit (months / weeks / days — your choice). The renderer auto-scales the time axis from 0 to a nice-round max.",
  "    · `unit` (e.g. `mo`, `w`, `d`) is a short suffix on the axis ticks and bar durations.",
  "    · `section` is optional but recommended — phases with the same section are grouped under a section label on the left.",
  "    · `focal` matches a phase label exactly; that bar renders in the spine accent.",
  "    · 2–10 phases. **Complexity floor stays**: a 2-phase gantt is a bullet list with bars; only emit when ≥ 4 tasks total AND ≥ 2 sections.",
  "    · Phase labels ≤ 24 chars (renderer truncates). Avoid colons / brackets.",
  "",
  "  ### sequenceDiagram · actor interactions over time (renders as `swimlane`)",
  ...
  "      \"focalLane\": \"{optional · lane name to tint as the focal row}\",",
  "      \"focal\": \"{optional · step label to render in accent}\",",
  "      \"steps\": [",
  ...
  "    ```",
  "  Hard rules:",
  "    · `type` MUST be `swimlane` (sequence diagrams are rendered through the unified swimlane pipeline).",
  "    · **Complexity floor · ≥ 2 lanes AND ≥ 4 steps**. A 3-step sequence is just a bullet list with arrow glyphs — drop it and use prose.",
  "    · 2–4 lanes; 3–10 steps total.",
  "    · `laneIdx` is the 0-based index into the `lanes` array. The renderer routes elbow connectors when consecutive steps cross lanes.",
  "    · `kicker` (optional) is a short mono caption inside the step box (the role or call type). Defaults to `STEP NN` when omitted.",
  "    · `focal` matches a step label exactly · accent border + accent kicker. `focalLane` tints the lane background.",
  "    · Labels ≤ 18 chars (renderer truncates).",
  "",
  "  ### journey · user / stakeholder journey scoring (renders as `swimlane`)",
  ...
  "        { \"label\": \"First task\",      \"laneIdx\": 1, \"kicker\": \"BUYER · IT\", \"score\": 1 },",
  ...
  "    · `score` is OPTIONAL · 1–5 (5 = best). Omit on steps where scoring doesn't apply. Mixing scored and unscored steps in one chart is fine.",
  "    · `kicker` can list multiple actors with ` · ` separator (e.g. `BUYER · IT`).",
  "    · For journey, set `focal` on the step with the *worst* score (the friction point that drives the argument). Counter-intuitive but correct — the editorial accent goes where the work is.",
  "",
  "  ### stateDiagram · lifecycle / phase transitions (renders as `state-machine`)",
  ...
  "      \"showStart\": true,",
  "      \"showEnd\": true,",
  "      \"focal\": \"{optional · state label rendered in accent}\",",
  "      \"states\": [",
  ...
  "    ```",
  "  Hard rules:",
  "    · `type` MUST be `state-machine`.",
  "    · **Complexity floor · ≥ 3 states AND at least one back-transition** (a `backTransitions` entry). A pure linear `Discovery → Pilot → Scale` chain has no cycle — render as `flowchart linear-v` instead. The thing that justifies a state-machine is the back-transition.",
  "    · `states` array: 2–6 entries. Each has `label` (≤ 16 chars), optional `hint` (≤ 14 chars · mono caption inside the box), optional `terminal: true` (draws with darker ink border).",
  "    · `forwardLabels` is an array indexed by the source state position · entry `i` labels the arrow from state `i` to state `i+1`. Empty / missing entries leave the arrow unlabeled.",
  "    · `backTransitions`: each entry needs `fromIdx > toIdx` (back-arcs go right-to-left). The renderer draws a dashed Q-curve above the row.",
  "    · `showStart: true` draws a solid black dot before the first state (the entry pseudo-state). `showEnd: true` draws a double-circle terminator after the last state.",
  "    · `focal` matches a state label exactly · that state renders in the spine accent.",
  "",
  "  ## Recommendations",
  ...
  "    ```",
  "    Hard rules:",
  "      · The block opens with the literal three backticks + `metric-strip` and closes with three backticks on a line by itself. Just like kami-chart blocks.",
  ...
  "· Markdown only — fenced ```kami-chart blocks (and the other structured-fence languages like ```metric-strip / ```callout) are part of markdown for our renderer.",
